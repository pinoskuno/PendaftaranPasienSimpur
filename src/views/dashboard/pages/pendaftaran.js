    ...
    <section class="container pb-3" style="margin: 1rem auto;">
      <div class="d-grid justify-content-center">
        <div class="col-12 container p-3">
          ...
          <!-- Dropdown untuk filter nama poli -->
          <label for="filterPoli">Filter by Poli:</label>
          <select id="filterPoli">
            <option value="">All Poli</option>
            <!-- Isi dropdown dengan nama-nama poli dari data -->
            <!-- Contoh: -->
            ${await generatePoliOptions()}
          </select>
        </div>
      </div>
...
`;
  ...
  async afterRender() {
    ...
    document.getElementById('filterPoli').addEventListener('change', function () {
      const selectedPoli = this.value.toLowerCase();
      filterAndRenderProductsByPoli(selectedPoli, productsContainer, productsHTML);
    });
...
// Fungsi filter berdasarkan nama poli
function filterAndRenderProductsByPoli(selectedPoli, container, productsHTML) {
  const filteredProducts = productsHTML.filter(html => {
    const productName = html.toLowerCase();
    return productName.includes(selectedPoli);
  });
...
async function generatePoliOptions() {
  const uniquePoliNames = new Set(); // Gunakan Set untuk mendapatkan nilai unik
  const fetchedDataProduct = await dataProduct._fetchAllDataProduct();

  fetchedDataProduct.forEach((d) => {
    const data = d.data();
    const namaPoli = data.nama_poli.toLowerCase();
    uniquePoliNames.add(namaPoli);
  });

  // Buat opsi dropdown berdasarkan nilai unik nama_poli
  let optionsHTML = "";
  uniquePoliNames.forEach((namaPoli) => {
    optionsHTML += `<option value="${namaPoli}">${namaPoli}</option>`;
  });
