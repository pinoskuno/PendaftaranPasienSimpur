...
const checkinAdmin = {
  async render() {
    return `
      ...
      <div class="table-data">
        ...
        <input type="datetime-local" id="filterEndDate" step="1">
          ...
          <select id="filterPoli">
            ...
            <!-- Tambahkan opsi lainnya sesuai dengan nama poli yang ada -->
          </select>
  ...
  async afterRender() {
    ...
    const isDateMatching = (dbDate, startDate, endDate) => {
      const dbDateTime = new Date(dbDate);
      const startDateTime = new Date(startDate);
      const endDateTime = new Date(endDate);
  
      // Check if the date is within the range
      return dbDateTime >= startDateTime && dbDateTime <= endDateTime;
    };
    ...
    const renderFilteredPasien = ()=> {
      ...
      fetchedTranscations.forEach((d) => {
        ...
        if (
          (!selectedPoli || data.poli.toLowerCase() === selectedPoli) &&
          (!selectedStatus || data.status.toLowerCase() === selectedStatus) &&
          (!selectedStartDate || !selectedEndDate || isDateMatching(data.waktu_reservasi, selectedStartDate, selectedEndDate))
        ) {
