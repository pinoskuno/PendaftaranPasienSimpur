...
import hapusMenu2Function from "../../../scripts/utils/hapusMenu2Function";
import homeMenu2Functions from "../../../scripts/utils/homeMenu2Functions";
import tambahMenu2Function from "../../../scripts/utils/tambahMenu2Function";
...
const menuAdmin2 = {
        ...
        <section class="section accounts">
          <div class="table-data">
            ...
            <button type="button" class="btn btn-success btn-lg" data-bs-toggle="modal" data-bs-target="#addProducts"><i class="bi bi-plus-circle"></i> TAMBAH DATA <i class="bi bi-plus-circle"></i></button>
              <h5 class="card-title">Daftar Menu Kelompok 2</h5>
              <div class="table-responsive">
                <table class="table table-hover" id="menu2Table">
                  <thead>
                  ...
                  <tbody class="align-middle" id="bodyMenu2">
        ...
        <div class="modal fade" id="addProducts" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
                ...
                <div class="modal-dialog modal-dialog-centered modal-xl">
                    <div class="modal-content" id="form-edit">
                      <form action="#" method="post" id="tambahMenu2" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                      <div class="mb-3">
                                          ...
                                          <input type="text" class="form-control" name="namaMenu2" id="namaMenu2" aria-label="Nama Menu" placeholder="Masukkan nama menu...." required>
                                      </div>
                                      
                                    </div>
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="gambarMenu2" class="form-label">Menu</label>
                                            <input class="form-control" type="file" id="gambarMenu2" name="gambarMenu2" required accept=".jpg,.jpeg,.png">
                                    ...
                                    <div class="d-grid gap-2 mt-2">
                                        <button class="btn btn-primary" type="submit" id="btnTambah2">Tambah</button>
                                    </div>
                                </div>
                            </div>
  ...
  async afterRender() {
    ...
    const bodyMenu2 = document.querySelector("#bodyMenu2");
    const fetchedMenu2 = await homeMenu2Functions.getAllData();
    console.log(fetchedMenu2);
    let i = 1;

    fetchedMenu2.forEach(async (d) => {
      const data = d.data();
      data.id = d.id;
      bodyMenu2.innerHTML += `<tr>
          <th scope="row">${i}</th>
          <td id='adminName' value=${data.admin}>${data.admin}</td>
          <td >${data.nama_menu2}</td>
          ...
          <td class="text-center"><a href="#/editmenu2/${data.id}" role="button" class="btn btn-outline-secondary">Edit</a>
          <button type="button" class="btn btn-outline-danger" id="deleteMenu2" data-id="${data.id}">Del</button></td>
        </tr>`;
      console.log(data.gambar);
      i += 1;
    });

    await homeMenu2Functions.convertIDtoName();

    await tambahMenu2Function.init();

    await hapusMenu2Function.init();
