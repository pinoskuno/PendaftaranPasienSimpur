...
import hapusMenu3Function from "../../../scripts/utils/hapusMenu3Function";
import homeMenu3Functions from "../../../scripts/utils/homeMenu3Functions";
import tambahMenu3Function from "../../../scripts/utils/tambahMenu3Function";
...
const menuAdmin3 = {
    ...
    <button type="button" class="btn btn-success btn-lg" data-bs-toggle="modal" data-bs-target="#addProducts"><i class="bi bi-plus-circle"></i> TAMBAH MENU <i class="bi bi-plus-circle"></i></button>
        ...
        <section class="section accounts">
          <div class="table-data">
            <div class="card-body">
              <h5 class="card-title">Daftar Menu Kelompok 3</h5>
              <div class="table-responsive">
                <table class="table table-hover" id="menu3Table">
                  <thead>
                  ...
                  <tbody class="align-middle" id="bodyMenu3">
        ...
        <div class="modal fade" id="addProducts" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
                ...
                <div class="modal-dialog modal-dialog-centered modal-xl">
                    <div class="modal-content" id="form-edit">
                      <form action="#" method="post" id="tambahMenu3" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                      <div class="mb-3">
                                          ...
                                          <input type="text" class="form-control" name="namaMenu3" id="namaMenu3" aria-label="Nama Menu" placeholder="Masukkan nama menu...." required>
                                      </div>
                                      
                                    </div>
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="gambarMenu3" class="form-label">Menu</label>
                                            <input class="form-control" type="file" id="gambarMenu3" name="gambarMenu3" required accept=".jpg,.jpeg,.png">
                                    ...
                                    <div class="d-grid gap-2 mt-2">
                                        <button class="btn btn-primary" type="submit" id="btnTambah3">Tambah</button>
                                    </div>
                                </div>
                            </div>
  ...
  async afterRender() {
    ...
    const bodyMenu3 = document.querySelector("#bodyMenu3");
    const fetchedMenu3 = await homeMenu3Functions.getAllData();
    console.log(fetchedMenu3);
    let i = 1;

    fetchedMenu3.forEach(async (d) => {
      const data = d.data();
      data.id = d.id;
      bodyMenu3.innerHTML += `<tr>
          <th scope="row">${i}</th>
          <td id='adminName' value=${data.admin}>${data.admin}</td>
          <td >${data.nama_menu3}</td>
          ...
          <td class="text-center"><a href="#/editmenu3/${data.id}" role="button" class="btn btn-outline-secondary">Edit</a>
          <button type="button" class="btn btn-outline-danger" id="deleteMenu3" data-id="${data.id}">Del</button></td>
        </tr>`;
      console.log(data.gambar);
      i += 1;
    });

    await homeMenu3Functions.convertIDtoName();

    await tambahMenu3Function.init();

    await hapusMenu3Function.init();

    $("#menu3Table").DataTable({
